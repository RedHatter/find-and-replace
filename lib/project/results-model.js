...
module.exports = class ResultsModel {
  ...
  replace (pathsPattern, replacePattern, replacementPaths) {
    ...
    let replacement
    if (this.findOptions.useFunction) {
      const fn = vm.runInThisContext(replacePattern)
      replacement = (...args) => {
    ...
    const promise = Promise.all(replacementPaths.map(async file => {
      const {fd, readStream, writeStream} = await rw(file);
      readStream
        .pipe(replaceStream(this.regex, replacement))
        .pipe(writeStream);
      return new Promise((resolve, reject) => writeStream
        .on('finish', () => {
          this.replacedPathCount++
          this.emitter.emit('did-replace-path', {
            filePath: file
          })
          resolve()
        })
