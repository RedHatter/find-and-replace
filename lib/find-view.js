...
class FindView {
  ...
  handleReplaceEvents() {
    this.replaceEditor.onDidStopChanging(() => this.liveReplace());
    this.refs.replaceNextButton.addEventListener('click', this.replaceNext.bind(this));
    this.refs.replaceAllButton.addEventListener('click', this.replaceAll.bind(this));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'find-and-replace:replace-previous': this.replacePrevious.bind(this),
  ...
  liveReplace() {
    this.element.classList.remove('function-error');
  }
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      this.element.classList.remove('has-results', 'has-no-results', 'function-error');
  ...
  findError(error) {
    ...
    if(error.function) {
      this.element.classList.add('function-error');
    }
  }
